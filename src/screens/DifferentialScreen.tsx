...
function EmergencyBadge({ level, rs }: { level: EmergencyLevel; rs: ResponsiveScale }) {
  ...
  return (
    <View style={{
      ...
      <MaterialCommunityIcons name={EMERGENCY_ICONS[level]} size={12} color={color} />
      <Text style={{ fontSize: rs.font(10), fontWeight: '700', color }}>{EMERGENCY_LABELS[level]}</Text>
    </View>
  );
}
    ...
    >
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: rs.space(8) }}>
        ...
        <View style={{
          ...
          backgroundColor: rank <= 3 ? systemColor + '20' : colors.border + '40',
          alignItems: 'center',
        ...
        {/* Name + system */}
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: rs.font(14), fontWeight: '700', color: colors.text }} numberOfLines={1}>
            {result.pathologyName}
          </Text>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: rs.space(4), marginTop: rs.space(2) }}>
            <MaterialCommunityIcons
              name={BODY_SYSTEM_ICONS[result.bodySystemId]}
              size={12}
              color={systemColor}
            />
            <Text style={{ fontSize: rs.font(11), color: systemColor, fontWeight: '600' }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: rs.space(8) }}>
        ...
        <Text style={{ fontSize: rs.font(13), fontWeight: '800', color: barColor, minWidth: rs.space(36), textAlign: 'right' }}>
          {pct}%
        </Text>
      </View>

      {/* Matched symptoms */}
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: rs.space(4), marginTop: rs.space(8) }}>
        {result.matchedSymptoms.map((s, i) => (
          <View key={i} style={{
            ...
            <Text style={{ fontSize: rs.font(10), color: barColor, fontWeight: '600' }} numberOfLines={1}>
              {s}
            </Text>
          </View>
        ))}
...
export function DifferentialScreen({ navigation }: Props) {
  ...
  const handleNavigateToPathology = useCallback((pathologyId: string, pathologyName: string) => {
    ...
    return (
      <View style={styles.container}>
        ...
        <ScrollView
          ...
          contentContainerStyle={{ paddingTop: rs.space(SPACING.md), paddingBottom: insets.bottom + rs.space(SPACING.xxxl) }}
        >
          {/* Summary */}
          <View style={[neuCard(colors), {
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: rs.space(SPACING.sm) }}>
              <MaterialCommunityIcons name="magnify-scan" size={20} color={colors.primary} />
              <Text style={{ fontSize: rs.font(14), fontWeight: '700', color: colors.text, marginLeft: rs.space(8) }}>
                {selectedCount} síntomas seleccionados
              </Text>
            </View>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: rs.space(4) }}>
              {Array.from(selectedSymptomIds).map(id => {
                const s = filteredSymptoms.find(x => x.id === id) ?? { label: id };
                return (
                  <View key={id} style={{
                    ...
                    <Text style={{ fontSize: rs.font(11), color: colors.primary, fontWeight: '600' }}>
                      {s.label}
                    </Text>
                  </View>
                );
          ...
          <View style={{
            ...
            <Text style={{ fontSize: rs.font(15), fontWeight: '700', color: colors.text }}>
              {results.length} patologías compatibles
            </Text>
          </View>

          {results.length === 0 && (
            <View style={{ alignItems: 'center', paddingVertical: rs.space(SPACING.xxxl) }}>
              <MaterialCommunityIcons name="magnify-close" size={48} color={colors.textLight} />
              <Text style={{ fontSize: rs.font(14), color: colors.textSecondary, textAlign: 'center', marginTop: rs.space(SPACING.md) }}>
          ...
          {results.map((r, idx) => (
            <ResultCard
              ...
              result={r}
              rank={idx + 1}
              onPress={() => handleNavigateToPathology(r.pathologyId, r.pathologyName)}
              colors={colors}
              rs={rs}
            />
          ))}
          ...
          {/* Back button */}
          <TouchableOpacity
            style={[styles.analyzeButton, { marginTop: rs.space(SPACING.md), backgroundColor: colors.textSecondary }]}
            onPress={handleBack}
            activeOpacity={0.8}
          >
            <MaterialCommunityIcons name="arrow-left" size={20} color="#FFFFFF" style={{ marginRight: rs.space(8) }} />
    ...
    <View style={styles.container}>
      ...
      >
        <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
          ...
          <Text style={styles.headerSubtitle}>Seleccioná síntomas para identificar patologías</Text>
        </Animated.View>
      </LinearGradient>

      <ScrollView
        showsVerticalScrollIndicator={false}
        contentContainerStyle={{ paddingTop: rs.space(SPACING.lg), paddingBottom: insets.bottom + rs.space(SPACING.xxxl) }}
      >
        {/* System filter chips */}
        <View style={{ paddingHorizontal: rs.space(SPACING.lg), marginBottom: rs.space(SPACING.md) }}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={{ flexDirection: 'row', gap: rs.space(SPACING.sm) }}>
              <TouchableOpacity
                ...
                style={[neuPill(colors), {
                  paddingHorizontal: rs.space(SPACING.md),
                  paddingVertical: rs.space(SPACING.sm),
                  backgroundColor: !systemFilter ? colors.primary + '20' : colors.neuSurface,
                  borderWidth: !systemFilter ? 1.5 : 0.5,
                  borderColor: !systemFilter ? colors.primary : colors.neuBorderDark,
                }]}
              >
                <Text style={{
                  fontSize: rs.font(12),
                  fontWeight: !systemFilter ? '700' : '500',
                  color: !systemFilter ? colors.primary : colors.textSecondary,
                }}>
                  Todos
                </Text>
              </TouchableOpacity>
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[neuPill(colors), {
                      ...
                      paddingHorizontal: rs.space(SPACING.md),
                      paddingVertical: rs.space(SPACING.sm),
                      backgroundColor: active ? sysColor + '20' : colors.neuSurface,
        ...
        {selectedCount > 0 && (
          <View style={{
            ...
            <Text style={{ fontSize: rs.font(13), fontWeight: '700', color: colors.primary }}>
              {selectedCount} seleccionados
            </Text>
            <TouchableOpacity onPress={clearSelection}>
              <Text style={{ fontSize: rs.font(13), fontWeight: '600', color: colors.error }}>Limpiar</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        <View style={{
            ...
            return (
              ...
              >
                ...
                <View style={{
                  ...
                  backgroundColor: s.isSign ? '#3B82F6' + '40' : '#F59E0B' + '40',
                }} />
              </TouchableOpacity>
            );
          })}
          ...
          {visibleSymptoms.length === 0 && (
            <Text style={{ fontSize: rs.font(13), color: colors.textSecondary, paddingVertical: rs.space(SPACING.lg) }}>
        ...
        <View style={{
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: rs.space(4) }}>
            <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: '#3B82F6' + '60' }} />
            <Text style={{ fontSize: rs.font(11), color: colors.textSecondary }}>Signo</Text>
          </View>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: rs.space(4) }}>
            <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: '#F59E0B' + '60' }} />
            <Text style={{ fontSize: rs.font(11), color: colors.textSecondary }}>Síntoma</Text>
          </View>
        </View>

        {/* Analyze button */}
        <TouchableOpacity
          style={[styles.analyzeButton, { opacity: selectedCount < 2 ? 0.5 : 1 }]}
          onPress={handleAnalyze}
          activeOpacity={0.8}
          disabled={selectedCount < 2}
        >
          <MaterialCommunityIcons name="stethoscope" size={20} color="#FFFFFF" style={{ marginRight: rs.space(8) }} />
          <Text style={styles.analyzeButtonText}>
            {selectedCount < 2 ? 'Seleccioná al menos 2 síntomas' : `Analizar ${selectedCount} síntomas`}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
