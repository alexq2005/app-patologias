...
import type { RootStackParamList, EmergencyProtocol, ProtocolCategory, ProtocolPriority } from '../types';
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: rs.space(SPACING.sm) }}>
        ...
        <View style={{
          ...
          <Text style={{ fontSize: rs.font(10), fontWeight: '800', color: priorityColor, letterSpacing: 0.5 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: rs.space(SPACING.sm) }}>
        <View style={{
          ...
          <Text style={{ fontSize: rs.font(12), fontWeight: '600', color: catColor }}>
      ...
      <View style={{
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: rs.space(6) }}>
          ...
          <Text style={{ fontSize: rs.font(12), color: colors.error, fontWeight: '600' }}>
            {item.banderasRojas.length} señal{item.banderasRojas.length !== 1 ? 'es' : ''} de alarma
          </Text>
        </View>
...
export function EmergencyProtocolsScreen({ navigation }: Props) {
  ...
  const [activeCategory, setActiveCategory] = useState<ProtocolCategory | 'todos'>('todos');
      ...
      <View>
        ...
        >
          {categories.map(cat => {
            ...
            const catColor =
              cat === 'todos' ? colors.primary : (PROTOCOL_COLORS[cat] ?? colors.primary);
            return (
              <TouchableOpacity
                key={cat}
              ...
              >
                ...
                >
                  {cat === 'todos' ? 'Todos' : CATEGORY_LABELS[cat as ProtocolCategory]}
                </Text>
              </TouchableOpacity>
            );
          })}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <MaterialCommunityIcons name="hospital-building" size={48} color={colors.textLight} />
